...
//     x: 0, y: 0
// }
// 2-2 background 이동을 위한 요소 선택 및 변수 선언
...
function backgroundMove() {
    ...
    // 여러개의 동일 값을 수정할 때는, 개별적으로 하지 말고, 일괄수정하면 개별오타가 안나고 디버깅포인트 or 작업 시점에 따른 반추가 쉽다.
    $background.setAttribute('style', `background-position: ${backgroundLocation.x}px ${backgroundLocation.y}px;`)
...
const animationFunctionMap = {
    ArrowDown: (function down() {
        // 익명함수를 리턴하는 즉시실행 함수
        return function () {
            // if (backgroundLocation.y >= mapSizeY) {return;}
    ...
    ArrowUp: (function up() {
        // 익명함수를 리턴하는 즉시실행 함수
        return function () {
            // if (backgroundLocation.y <= 0) {return;}
            backgroundLocation.y += stepSize;
            backgroundMove()
        }
    })(),
    ArrowLeft: (function left() {
        // 익명함수를 리턴하는 즉시실행 함수
        return function () {
            // if (backgroundLocation.x <= 0) {return;}
    ...
    ArrowRight: (function right() {
        // 익명함수를 리턴하는 즉시실행 함수
        return function () {
            // if (backgroundLocation.x >= mapSizeX) {return;}
            backgroundLocation.x -= stepSize;
            backgroundMove()
        }
    })()
...
document.addEventListener('keyup', (evt) => {
        ...
        // 3-2 애니메이션 중단
        // pauseWalk(evt);
        // 캐릭터 걸음 애니메이션 중단이 배겨여이동 완료 전에 이루어져서 미끄러짐 효과가 발생
